...
let isStartingGame = true
let winner;
let isMatchDrew = false
...
function drawCard(activeplayer) {
    ...
    card.src = `./PNG/${currentCard}.png`;
    document.querySelector(activeplayer['div']).appendChild(card);
    if(isStartingGame && activeplayer.div == '#dealer-box'){
        let card2 = document.createElement('img');
        card2.src = `./PNG/gray_back.png`;
        document.querySelector(activeplayer['div']).appendChild(card2);
    }
    hitsound.play();
...
function BJhit(){
    if(winner != You && winner != Dealer && !isMatchDrew){
        if(You['score']<=21){
            if(isStartingGame){
                drawCard(You);
                drawCard(You);
